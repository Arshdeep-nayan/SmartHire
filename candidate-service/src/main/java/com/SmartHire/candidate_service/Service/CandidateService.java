...
public class CandidateService {
    ...
    public CandidateService(CandidateRepository repo, CandidateEventProducer producer) {
        this.repo = repo;
    ...
    public Candidate registerCandidate(Candidate candidate) {
        ...
        if (repo.findByEmail(candidate.getEmail()).isPresent()) {
            log.warn("Candidate already exists with email: {}", candidate.getEmail());
    ...
    public Candidate updateCandidate(int id, Candidate candidate) {
        ...
        Candidate updatedCandidate = repo.save(existing);
    ...
    public void deleteCandidate(int id) {
        ...
        repo.findById(id).orElseThrow(() -> new CandidateNotFoundException(
    ...
    public Candidate activateCandidate(int id) {
        ...
        Candidate existing = repo.findById(id)
                .orElseThrow(() -> new CandidateNotFoundException("Candidate not found"));
        existing.setStatus(Candidate.CandidateStatus.ACTIVE);
    ...
    public Candidate deactivateCandidate(int id) {
        ...
        Candidate existing = repo.findById(id)
                .orElseThrow(() -> new CandidateNotFoundException("Candidate not found"));
        existing.setStatus(Candidate.CandidateStatus.INACTIVE);
        Candidate updated = repo.save(existing);
    ...
    public Candidate markCandidateAsHired(int id) {
        ...
        Candidate existing = repo.findById(id)
                .orElseThrow(() -> new CandidateNotFoundException("Candidate not found"));
        existing.setStatus(Candidate.CandidateStatus.HIRED);
        Candidate updated = repo.save(existing);
    ...
    public List<Candidate> getCandidatesSortedByDate() {
        ...
        List<Candidate> candidates = repo.findAll(Sort.by(Sort.Direction.DESC, "registeredAt"));
    ...
    public void applyForJob(int candidateId, int jobId) {
        ...
        Candidate candidate = repo.findById(candidateId)
                .orElseThrow(() -> new CandidateNotFoundException("Candidate not found"));
        CandidateAppliedEvent event = new CandidateAppliedEvent(candidate.getId(), jobId);
