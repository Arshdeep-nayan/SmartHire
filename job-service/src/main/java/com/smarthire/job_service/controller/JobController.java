...
public class JobController {
    ...
    public ResponseEntity<job> createJob(@Valid @RequestBody job newJob) {
        return ResponseEntity.status(HttpStatus.CREATED).body(service.createJob(newJob));
    }

    @PutMapping("/job/update/{id}")
    public ResponseEntity<job> updateJob(@PathVariable int id, @Valid @RequestBody job updatedJob) {
        return ResponseEntity.ok(service.updateJob(id, updatedJob));
    }
